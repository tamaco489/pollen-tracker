...
func rowToSymptom(row gen.Symptom) (dto.GetSymptoms, error) {
	...
	if err != nil {
		return dto.GetSymptoms{}, fmt.Errorf("parse uuid: %w", err)
	}
	date, err := time.Parse("2006-01-02", row.Date)
	if err != nil {
		return dto.GetSymptoms{}, fmt.Errorf("parse date: %w", err)
	}
	createdAt, err := time.Parse(time.RFC3339Nano, row.CreatedAt)
	if err != nil {
		return dto.GetSymptoms{}, fmt.Errorf("parse created_at: %w", err)
	}
	updatedAt, err := time.Parse(time.RFC3339Nano, row.UpdatedAt)
	if err != nil {
		return dto.GetSymptoms{}, fmt.Errorf("parse updated_at: %w", err)
	}
