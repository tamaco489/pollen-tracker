...
│   ├── cmd/api/                # Lambda エントリポイント (Echo v5)
│   ├── cmd/authorizer/         # Lambda Authorizer (x-api-key 検証)
│   ├── internal/
│   │   ├── di/                 # 依存性注入コンテナ
...
│   │   ├── infrastructure/     # リポジトリ実装・sqlc 生成コード
...
│   │   ├── library/            # 外部 API クライアント (Google Pollen API 等)
│   │   ├── logger/             # 構造化ロガー (slog)
│   │   └── utils/              # ユーティリティ関数
│   ├── tools/
│   │   ├── httprequest/        # VS Code REST Client テストファイル
│   │   ├── migrations/         # DB マイグレーション (goose SQL ファイル・実行ラッパー)
...
│   ├── config/                 # 環境設定 (EnvConfig)
│   ├── lib/
│   │   ├── stacks/             # スタック定義
│   │   └── constructs/         # カスタム L3 コンストラクト
│   └── test/                   # Jest ユニットテスト
│
├── .github/
