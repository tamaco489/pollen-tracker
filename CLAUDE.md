...
- **データベース**: Turso (libSQL) + goose (マイグレーション) + sqlc (型安全クエリ生成)
- **コード生成**: oapi-codegen (OpenAPI → Go), sqlc (SQL → Go)
...
- 自動生成ファイルの手動編集禁止:
  - `backend/internal/gen/*.gen.go` (oapi-codegen — `make gen-api` で再生成)
  - `backend/internal/infrastructure/datastore/gen/` (sqlc — `make gen-sqlc` で再生成)
- 機密情報のハードコーディング禁止 (API キー, トークン, 接続情報)
- 絵文字の使用禁止 (明示的に求められた場合を除く)
- **インフラデプロイ・Lambda/S3 操作の禁止** — 以下のコマンドはユーザーのみが実行する。Claude が実行してはならない:
  - `cdk deploy` / `cdk destroy`
  - `make upload-api` / `make upload-authorizer`
  - `make deploy-api` / `make deploy-authorizer`
  - `aws s3 cp` (Lambda 成果物アップロード)
